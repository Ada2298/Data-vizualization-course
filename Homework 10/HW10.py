...
def survival(selected_categories):
	categories = {'lightweight': ['Capomulin','Ramicane'],
					'placebo': ['Placebo'],
					'heavyweight': ['Ceftamin', 'Infubinol', 'Ketapril', 
								'Naftisol', 'Propriva', 'Stelasyn','Zoniferol']}
	...
	traces = []
	for category in output_options:
		traces.append(go.Scatter(x=merged_df[merged_df['Drug Regimen']==category].groupby('Timepoint').count().index.values,
								  y = merged_df[merged_df['Drug Regimen']==category].groupby('Timepoint').count()['Mouse ID'].values,
									name = category,
									customdata=[(category,i) for i in np.arange(0,45,5)],
									mode = 'lines + markers',
									marker = dict(color=drug_colors[category])
						)
					)
